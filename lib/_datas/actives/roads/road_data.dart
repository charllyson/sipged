...
class RoadData extends ChangeNotifier {
  ...
  factory RoadData.fromDocument(DocumentSnapshot snapshot) {
    if (!snapshot.exists) throw Exception("Documento não encontrado");
    final data = snapshot.data() as Map<String, dynamic>?;
    if (data == null) throw Exception("Dados estão vazios");
    return RoadData.fromMap(data, id: snapshot.id);
  }

  /// Cria a partir de um map genérico
  factory RoadData.fromMap(Map<String, dynamic> map, {String? id}) {
    return RoadData(
      id: id ?? map['id'],
      acronym: map['acronym'],
      uf: map['uf'],
  ...
  /// Cor interpretada a partir do `colorHex`
  Color get color =>
      colorHex != null ? _hexToColor(colorHex!) : Colors.blueGrey;
...
extension RoadDataExtensions on RoadData {
