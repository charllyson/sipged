...
// ADJUSTMENT / REVISION (stores/modelos)
...
class OverviewDashboardList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final int? order = selected.order; // número/ordem da medição

    // Stores já carregadas (garanta ensureAllLoaded/initialize em nível superior)
    ...
    double _readRevisionValue() {
      ...
      RevisionMeasurementData? item = revStore.all.firstWhere(
            (e) => e.contractId == contractId && e.order != null && e.order == order,
        orElse: () => RevisionMeasurementData(),
      );

      if (item.contractId == null) {
        item = revStore.all.firstWhere(
              (e) => e.contractId == contractId,
          orElse: () => RevisionMeasurementData(),
        );
      }
                ...
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (_) => TabBarHiringPage(
                      ...
                      initialTabIndex: 4, // ajuste se necessário
                    ),
                  ),
                );
              },
                  ...
                  child: Wrap(
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          Chip(
                            label: Text('Número: ${order ?? selected.order ?? '-'}'),
