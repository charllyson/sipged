// lib/screens/process/measurement/adjustment/adjustment_measurement.dart
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<AdjustmentMeasurementCubit, AdjustmentMeasurementState>(
      ...
      builder: (context, state) {
        ...
        final labels =
        state.adjustments.map((m) => (m.order ?? 0).toString()).toList();
        final values =
        state.adjustments.map((m) => m.value ?? 0.0).toList();
        ...
        final double valorTotalDisponivel =
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: AdjustmentMeasurementFormSection(
                            ...
                            currentAdjustmentMeasurementId:
                            state.selected?.id,
                            ...
                            onSave: () async {
                              ...
                              final parsedOrder =
                              _parseInt(orderCtrl.text);
                              final effectiveOrder = (parsedOrder == null ||
                                  parsedOrder <= 0)
                                  ? _computeNextOrder(state)
                                  : parsedOrder;

                              final value =
                              _parseCurrency(valueCtrl.text);
                              final date =
                              _parseDate(dateCtrl.text);
                            ...
                            onDeleteSideItem: (i) async {
                              ...
                              if (i < 0 || i >= atts.length) return;
                              atts.removeAt(i);
                              await cubit.updateAttachments(atts);
                            },
                        ...
                        AdjustmentMeasurementTableSection(
                          onTapItem:
                              (AdjustmentMeasurementData data) {
                            final idx =
                            state.adjustments.indexWhere(
                                  (e) => e.id == data.id,
                            );
                            ...
                            try {
                              ...
                              NotificationCenter.instance.show(
                                AppNotification(
                                  type: AppNotificationType.error,
                                  title: const Text(
                                      'Erro ao apagar reajuste'),
                                  subtitle: Text('$e'),
                                ),
                              );
                            }
                          },
                          adjustmentMeasurementsData:
                          state.adjustments,
