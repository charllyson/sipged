...
import 'package:sipged/_blocs/modules/financial/payments/revision/revision_paid_state.dart';
...
import 'package:sipged/screens/modules/financial/payments/revision/revision_payments_list.dart';

class RevisionPaymentFormView extends StatefulWidget {
  const RevisionPaymentFormView({
    super.key,
  ...
  @override
  State<RevisionPaymentFormView> createState() {
    return _RevisionPaymentFormViewState();
  }
}

class _RevisionPaymentFormViewState extends State<RevisionPaymentFormView> {
  ...
  String get _revisionId {
    return _s(widget.selectedRevisionMeasurement.id);
  }

  double get _revisionValue {
    final value = widget.selectedRevisionMeasurement.value ?? 0.0;
  ...
  int? get _revisionOrder {
    final fromModel = widget.selectedRevisionMeasurement.order;
  ...
  void initState() {
    ...
    _orderCtrl = TextEditingController(
      text: _revisionOrder?.toString() ?? widget.orderController.text.trim(),
    );
  ...
  @override
  void didUpdateWidget(covariant RevisionPaymentFormView oldWidget) {
    super.didUpdateWidget(oldWidget);

    final nextOrder =
        _revisionOrder?.toString() ?? widget.orderController.text.trim();

    if (_orderCtrl.text != nextOrder) {
      _orderCtrl.text = nextOrder;
    }

    final oldRevisionId = _s(oldWidget.selectedRevisionMeasurement.id);
    final nextRevisionId = _s(widget.selectedRevisionMeasurement.id);

    if (oldRevisionId != nextRevisionId) {
  ...
  Future<void> _loadDfdSummary() async {
    ...
    try {
      ...
      debugPrint('Falha ao carregar DFD para resumo do pagamento de revisão: $e');
  ...
  double _sumPaymentTotalValues(List<RevisionPaidData> payments) {
  ...
  Future<void> _showLocalPaymentError(String message) async {
    ...
    await NotificationPayments.show(
      ...
      details: message,
      kind: NotificationPaymentKind.revision,
      ...
      action: 'revision_payment_validation_error',
      extra: <String, dynamic>{
        'action': 'revision_payment_validation_error',
        'paymentType': 'revision_payment',
        'sourceModule': 'financial_payments_revision',
        'tenantId': tenantId,
        'companyId': tenantId,
        ...
        'descricaoObjeto': _contractSummary,
        'nomeDemanda': _contractSummary,
        'revisionOrder': _revisionOrder?.toString(),
        'revisionValue': _revisionValue,
      },
    );
  }
  ...
  void _fillFromPayment(RevisionPaidData? payment) {
    if (!mounted) return;
  ...
  Widget _dateInput({
    ...
    return DateFieldChange(
      key: ValueKey<String>(
        'revision-payment-date-$keyId-$_formNonce-${controller.text}',
      ),
      width: width,
  ...
  Future<void> _notifyPaymentSaved({
    ...
    final revisionOrderText = _revisionOrder?.toString().trim() ?? '';
    ...
    final title = revisionOrderText.isNotEmpty
        ? 'Pagamento da revisão $revisionOrderText $actionLabel'
        : 'Pagamento de revisão $actionLabel';
    ...
    await NotificationPayments.show(
      ...
      subtitle: _contractSummary,
      details: wasEditing
          ? 'O pagamento da revisão foi atualizado.'
          : 'O pagamento da revisão foi registrado.',
      kind: NotificationPaymentKind.revision,
      ...
      descricaoObjeto: _contractSummary,
      nomeDemanda: _contractSummary,
      action: wasEditing ? 'revision_payment_updated' : 'revision_payment_created',
      ...
      paymentMeasurementNumber: revisionOrderText,
      paymentMeasurementOrder: revisionOrderText,
      paymentMeasurementDate: widget.selectedRevisionMeasurement.date,
      paymentMeasurementValue: _revisionValue,
      measurementNumber: revisionOrderText,
      measurementOrder: revisionOrderText,
      measurementDate: widget.selectedRevisionMeasurement.date,
      measurementValue: _revisionValue,
      extra: <String, dynamic>{
        'action': wasEditing
            ? 'revision_payment_updated'
            : 'revision_payment_created',
        'paymentType': 'revision_payment',
        'sourceModule': 'financial_payments_revision',
        'tenantId': tenantId,
        'companyId': tenantId,
        ...
        'revisionOrder': revisionOrderText,
        'paymentRevisionOrder': revisionOrderText,
  ...
  Future<void> _notifyPaymentDeleted({
    ...
    final revisionOrderText =
    payment.revisionOrder?.toString().trim().isNotEmpty == true
        ? payment.revisionOrder.toString().trim()
        : _revisionOrder?.toString().trim() ?? '';
    ...
    final title = revisionOrderText.isNotEmpty
        ? 'Pagamento da revisão $revisionOrderText excluído'
        : 'Pagamento de revisão excluído';
    ...
    await NotificationPayments.show(
      ...
      title: title,
      subtitle: _contractSummary,
      details: 'O pagamento da revisão foi excluído.',
      kind: NotificationPaymentKind.revision,
      status: NotificationStatus.warning,
      delivery: NotificationDelivery.localBellAndPush,
      saveInBell: true,
      ...
      descricaoObjeto: _contractSummary,
      nomeDemanda: _contractSummary,
      action: 'revision_payment_deleted',
      paymentDate: payment.paymentDate,
      paymentValue: payment.paymentValue,
      paymentMainValue: payment.paymentValue,
      paymentRetentionsValue: retentionsValue,
  ...
  Future<void> _savePayment({
    required RevisionPaidCubit cubit,
    required RevisionPaidData? selected,
  ...
  Widget build(BuildContext context) {
    return BlocListener<RevisionPaidCubit, RevisionPaidState>(
      ...
      child: BlocBuilder<RevisionPaidCubit, RevisionPaidState>(
            ...
            builder: (context, tenantState) {
              ...
              return LayoutBuilder(
                builder: (context, constraints) {
                  final cubit = context.read<RevisionPaidCubit>();
                  final tenantCubit = context.read<TenantCubit>();
                  ...
                  if (paymentState.status == RevisionPaidStatus.loading &&
                  ...
                  if (paymentState.status == RevisionPaidStatus.failure &&
                      paymentState.payments.isEmpty) {
                    return Center(
                      child: Padding(
                  ...
                  final saldo = _roundMoney(_revisionValue - totalPago);
                  ...
                  final camposWrap = Wrap(
                    ...
                    children: [
                      ...
                      DropDownChange(
                        ...
                        key: ValueKey<String>(
                          'revision-payment-funding-$_fundingNonce-${_fonteCtrl.text}',
                        ),
                        width: inputsWidth,
