...
class _ActiveOaesFormState extends State<ActiveOaesForm> {
  ...
  void _fillUiFromForm(ActiveOaesData f) {
    ...
    _setIfDiff(
      _dateCtrl,
      f.lastDateIntervention != null
          ? _fmtDDMMYYYY(f.lastDateIntervention!)
          : '',
    );
  ...
  ActiveOaesData _patchFromUi(ActiveOaesData base) {
    return base.copyWith(
      order: int.tryParse(_orderCtrl.text.trim()),
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ActiveOaesCubit, ActiveOaesState>(
      ...
      builder: (context, st) {
        ...
        final patched = _patchFromUi(st.form);
        final canSave = !st.saving && _requiredValid(patched);
        ...
        final botoes = Row(
          ...
          children: [
            TextButton.icon(
              onPressed: canSave
                  ? () {
                final data = _patchFromUi(st.form);
                cubit.upsert(data);

                _showNotification(
                  title: 'Enviando...',
                  subtitle: st.selectedIndex != null
            ...
            if (st.selectedIndex != null)
              TextButton.icon(
                ...
                onPressed: st.saving ? null : cubit.clearSelection,
              ),
          ],
        );
