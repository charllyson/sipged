            ...
            children: [
              ...
              ListView.separated(
                ...
                itemBuilder: (ctx, i) {
                  final acc = items[i];
                  final typeCanonical =
                  AccidentsData.canonicalType(acc.typeOfAccident);

                  final colorType =
                  AccidentsData.getColorByAccidentType(typeCanonical);
                  final iconType = AccidentsData.iconFor(typeCanonical);

                  final rod = (acc.highway ?? '').trim();
                  final rodLabel = rod.isEmpty ? 'Rodovia não informada' : rod;

                  return _AccidentCard(
                    colorType: colorType,
                    iconType: iconType,
                    title:
                    '${AccidentsData.getTitleByAccidentType(typeCanonical)} · $rodLabel',
                    subtitle: (acc.location?.trim().isNotEmpty == true)
                        ? acc.location!.trim()
                        : (acc.referencePoint?.trim().isNotEmpty == true
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.search_off_rounded, size: 56, color: cs.onSurfaceVariant),
