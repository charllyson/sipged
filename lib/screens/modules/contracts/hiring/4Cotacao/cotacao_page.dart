...
class _CotacaoPageState extends State<CotacaoPage>
  ...
  Future<void> _loadContract(String contractId) async {
    ...
    try {
      ...
      debugPrint(
        '[CotacaoPage] Erro ao carregar contrato | '
            'tenantId=$_tenantId | '
            'contractId=$cid | '
            'erro=$error',
      );
  ...
  Future<void> _loadDfdData(String contractId) async {
    ...
    try {
      ...
      debugPrint(
        '[CotacaoPage] Falha ao carregar DFD | '
            'tenantId=$_tenantId | '
            'contractId=$cid | '
            'erro=$error',
      );
  ...
  Future<void> _saveApproveAndNext() async {
    ...
    try {
      ...
      debugPrint(
        '[CotacaoPage] Erro em _saveApproveAndNext | '
            'tenantId=$_tenantId | '
            'contractId=$_contractId | '
            'erro=$error',
      );
  ...
  Future<void> _updateApproved() async {
    ...
    try {
      ...
      debugPrint(
        '[CotacaoPage] Erro em _updateApproved | '
            'tenantId=$_tenantId | '
            'contractId=$_contractId | '
            'erro=$error',
      );
                    ...
                    SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          const SizedBox(height: 12),
                          RepaintBoundary(
                            child: SectionConviteDivulgacao(
                              data: _formData,
                              isEditable: _isEditable,
                              onChanged: (updated) {
                                setState(() => _formData = updated);
                          ...
                          const SizedBox(height: 12),
                          RepaintBoundary(
                            child: SectionRespostasFornecedores(
                              data: _formData,
                              isEditable: _isEditable,
                          ...
                          const SizedBox(height: 12),
                          RepaintBoundary(
                            child: SectionConsolidacaoResultado(
                              data: _formData,
                              isEditable: _isEditable,
                              onChanged: (updated) {
                                setState(() => _formData = updated);
