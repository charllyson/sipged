...
class AdjustmentMeasurementGraphSection extends StatelessWidget {
  const AdjustmentMeasurementGraphSection({
    ...
    this.adjustmentIds = const <String>[],
    this.adjustmentOrders = const <int?>[],
  ...
  final List<int?> adjustmentOrders;
  ...
  /// Total disponível para reajustes.
  ///
  /// Neste caso: valor total de apostilamentos disponível.
  final double valorTotal;

  /// Valor consumido por reajustes.
  ...
  static const Color _adjustmentColor = Color(0xFFFB8323);
  static const Color _adjustmentTrackColor = Color(0xFFFFEDD5);
  ...
  List<double> _buildPaymentValuesFromPayments(int length) {
    ...
    final totalsByAdjustmentId = <String, double>{};
    final totalsByAdjustmentOrder = <int, double>{};
    ...
    for (final payment in payments) {
      ...
      final adjustmentId = payment.adjustmentId?.trim() ?? '';

      if (adjustmentId.isNotEmpty) {
        totalsByAdjustmentId[adjustmentId] =
            (totalsByAdjustmentId[adjustmentId] ?? 0.0) + value;
      }

      final order = payment.adjustmentOrder;

      if (order != null) {
        totalsByAdjustmentOrder[order] =
            (totalsByAdjustmentOrder[order] ?? 0.0) + value;
      }
    }

    return List<double>.generate(length, (index) {
      if (index < adjustmentIds.length) {
        final adjustmentId = adjustmentIds[index].trim();

        if (adjustmentId.isNotEmpty) {
          final valueById = totalsByAdjustmentId[adjustmentId];
      ...
      if (index < adjustmentOrders.length) {
        final order = adjustmentOrders[index];

        if (order != null) {
          final valueByOrder = totalsByAdjustmentOrder[order];
  ...
  Widget build(BuildContext context) {
    ...
    final safeAdjustmentValues = hasData
    ...
    final totalReajustado = _safeTotal(totalMedicoes);
    ...
    final percentualReajustado = _safePercent(
      value: totalReajustado,
            ...
            children: [
              ...
              RepaintBoundary(
                child: GaugeChartChange(
                  ...
                  footerLabel:
                  'Reajustado: ${SipGedFormatMoney.doubleToText(totalDisponivel)}',
                  widthGraphic: 260,
                  radius: 78,
                  centerMode: GaugeTextMode.percent,
                  centerSeriesId: 'adjustments',
                  ...
                  series: [
                    GaugeChartSeries(
                      id: 'adjustments',
                      label: 'Reajustes',
                      percent: percentualReajustado,
                      value: totalReajustado,
                      color: _adjustmentColor,
                      trackColor: _adjustmentTrackColor,
                    ),
                    GaugeChartSeries(
                      id: 'payments',
                      label: 'Pago',
              ...
              RepaintBoundary(
                child: DonutChartChanged(
                  ...
                  onTouch: (index) {
                    if (_isValidIndex(
                      index: index,
                      length: safeAdjustmentValues.length,
                      hasData: hasData,
                    )) {
              ...
              RepaintBoundary(
                child: LineChartChanged(
                  headerTitle: 'Reajustes x Pagamentos',
                  ...
                  series: [
                    LineSeries(
                      id: 'adjustments',
                      name:
                      'Reajustado: ${SipGedFormatMoney.doubleToText(totalReajustado)}',
                      values: safeAdjustmentValues,
                      labels: safeLabels,
                      color: _adjustmentColor,
                      showArea: true,
