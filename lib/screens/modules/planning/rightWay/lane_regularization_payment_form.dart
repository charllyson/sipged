// lib/screens/modules/planning/rightWay/property/lane_regularization_payment_form.dart
...
class LaneRegularizationPaymentForm extends StatefulWidget {
  ...
  @override
  State<LaneRegularizationPaymentForm> createState() =>
      _LaneRegularizationPaymentFormState();
}

class _LaneRegularizationPaymentFormState
    extends State<LaneRegularizationPaymentForm> {
  ...
  void _ensureSelectedIndexValid(int len) {
    if (_selectedDocIndex == null) return;
    if (len <= 0) {
      setState(() => _selectedDocIndex = null);
      return;
    }
    if (_selectedDocIndex! >= len) {
      setState(() => _selectedDocIndex = len - 1);
    }
  }
        ...
        return LayoutBuilder(
          builder: (context, constraints) {
              ...
              children: [
                Wrap(
                  ...
                  children: [
                    ...
                    CustomTextField(
                      width: w,
                      controller: controller.bankAgencyCtrl,
                      enabled: controller.isEditable,
                      labelText: 'Agência',
                    ),
                    CustomTextField(
                    ...
                    CustomTextField(
                      width: w,
                      controller: controller.rpvPrecCtrl,
                      enabled: controller.isEditable,
                      labelText: 'RPV/Precatório',
                    ),
                    CustomTextField(
                      width: w,
                      controller: controller.depositInCourtCtrl,
                      enabled: controller.isEditable,
                      labelText: 'Depósito em Juízo (R\$)',
                      keyboardType: TextInputType.number,
                      inputFormatters: [
                        CurrencyInputFormatter(
                          leadingSymbol: 'R\$ ',
                    ...
                    CustomTextField(
                      width: w,
                      controller: controller.ressettlementCtrl,
                      enabled: controller.isEditable,
                      labelText: 'Reassentamento necessário? (Sim/Não)',
                    ),
                    CustomTextField(
                      width: w,
                      controller: controller.familyCountCtrl,
                      enabled: controller.isEditable,
                      labelText: 'Nº de Famílias',
                      keyboardType: TextInputType.number,
                      inputFormatters: [FilteringTextInputFormatter.digitsOnly],
                    ),
                    CustomTextField(
                      width: w,
                      controller: controller.socialNotesCtrl,
                      enabled: controller.isEditable,
            ...
            final side = SideListBox(
              ...
              selectedIndex: _selectedDocIndex,
              width: sideWidth,
              onAddPressed: (controller.selected != null && controller.isEditable)
                  ? () => controller.addDocFile(context)
                  : null,
              onTap: (i) async {
                setState(() => _selectedDocIndex = i);
                await controller.openDocAt(context, i);
              },
              onDelete: controller.isEditable
                  ? (i) async {
                await controller.removeDocAt(context, i);
                if (!mounted) return;
                setState(() => _selectedDocIndex = null);
              }
                  : null,

              // ✅ rename embutido (padrão dos demais)
              enableRename: controller.isEditable,
              onItemsChanged: (items) {
                _ensureSelectedIndexValid(items.length);
              },
              onRenamePersist: ({
                required int index,
                ...
                try {
                  // ✅ precisa existir no controller (sem dialog)
                  await controller.renameDocLabel(
                    index: index,
