...
import 'package:sipged/screens/modules/planning/geo/geo_network_map_area.dart';
import 'package:sipged/screens/modules/planning/geo/geo_network_status_overlay.dart';
...
class _GeoNetworkViewState extends State<GeoNetworkView> {
  ...
  Object _buildMapVisualDataSignature({
    required FeatureState genericState,
    required LayerDataMap mapData,
  }) {
  ...
  Widget _buildMapContent({
    ...
    required ToolboxState measurementState,
    required FeatureState genericState,
    required MapCubit editorCubit,
  }) {
    final mapVisualDataSignature = _buildMapVisualDataSignature(
      genericState: genericState,
      mapData: mapData,
    );

    final map = GeoNetworkMapArea(
    ...
    final showFloatingStatus = !_statusDismissed && mapData.showFloatingStatus;
      ...
      children: [
        ...
        GeoNetworkStatusOverlay(
          ...
          onFinishDistanceMeasurement: () {
            context.read<ToolboxCubit>().clear();
            context.read<MapCubit>().selectTool(editorState.selectedToolId);
          },
          onFinalizeCurrentPointEditing:
          editorCubit.finalizeCurrentPointEditing,
          onCancelCurrentPointEditing: editorCubit.cancelCurrentPointEditing,
          onFinalizeCurrentLineEditing: editorCubit.finalizeCurrentLineEditing,
          onCancelCurrentLineEditing: editorCubit.cancelCurrentLineEditing,
    ...
    return MultiBlocListener(
      listeners: [
        ...
        BlocListener<FeatureCubit, FeatureState>(
          listenWhen: (p, c) {
            return p.error != c.error || p.selected != c.selected;
          },
          listener: (context, state) {
            if (state.error != null && state.error!.trim().isNotEmpty) {
              _showSnack(context, state.error!);
        ...
        BlocListener<MapCubit, MapState>(
          listenWhen: (p, c) {
            return p.selectedLayerPanelItemId != c.selectedLayerPanelItemId;
          },
      ...
      child: Builder(
        builder: (context) {
          ...
          final content = _buildMapContent(
            mapData: mapData,
            editorState: editorState,
            measurementState: measurementState,
            genericState: genericState,
          ...
          return Scaffold(
            ...
            drawer: LayerDrawer(
              ...
              onMoveUp: (id) {
                context.read<MapCubit>().moveLayerUp(
                  id,
                  mapData.currentTree,
                );
              },
              onMoveDown: (id) {
                context.read<MapCubit>().moveLayerDown(
                  id,
                  mapData.currentTree,
                );
              },
              ...
              onDropItem: (draggedId, targetParentId, targetIndex) {
                return context.read<MapCubit>().dropItem(
                  draggedId,
                  targetParentId,
                  targetIndex,
                  mapData.currentTree,
              ...
              onRenameSelected: (id) {
                return _editSelectedItem(
                  id,
                  mapData.currentTree,
                );
              },
              onRemoveSelected: (id) {
                return context.read<MapCubit>().removeSelectedItem(
                  id,
                  mapData.currentTree,
                );
              },
              onConnectLayer: (id) {
                return _handleConnectLayer(
                  id,
                  mapData.currentTree,
                );
              },
