...
enum AttributeTableMode { importFile, firestore }
...
class AttributeTable extends StatefulWidget {
  ...
  const AttributeTable({
    ...
    this.title,
    this.description,
    this.mode = AttributeTableMode.importFile,
  });
...
class _AttributeTableState extends State<AttributeTable> {
  ...
  void initState() {
    ...
    Future.microtask(() {
      ...
      if (widget.mode == AttributeTableMode.firestore) {
  ...
  Widget _overlaySaving(FeatureState state) {
    final progresso =
    (state.importProgress.clamp(0.0, 1.0) * 100).toStringAsFixed(1);

    final msg = state.importStatus == FeatureImportStatus.deleting
