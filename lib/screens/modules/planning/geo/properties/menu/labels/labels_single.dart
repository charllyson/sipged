...
class LabelsSingle extends StatefulWidget {
  ...
  final ValueChanged<List<LayerDataLabel>> onChanged;
...
class _LabelsSingleState extends State<LabelsSingle> {
  late List<LayerDataLabel> _layers;
  int _selectedIndex = 0;

  @override
  void initState() {
    super.initState();
    _layers = List<LayerDataLabel>.from(widget.labelLayers);
    _normalizeSelection();
  }
  ...
  void _notifyParent() {
    widget.onChanged(List<LayerDataLabel>.unmodifiable(_layers));
  }

  LayerDataLabel? get _selectedLayer {
    if (_layers.isEmpty) return null;
    return _layers[_selectedIndex];
  }

  List<LayerDataLabel> get _visualLayers =>
  ...
  void _updateSelected(LayerDataLabel value) {
  ...
  String _itemTitle(LayerDataLabel item, int sourceIndex) {
  ...
  List<LayerDataSimple> _buildPreviewLayers() {
    return _layers.map(_mapLabelToPreviewLayer).toList(growable: false);
  }

  LayerDataSimple _mapLabelToPreviewLayer(LayerDataLabel label) {
    final isPointLike =
  ...
  String _previewTextFor(LayerDataLabel label) {
    ...
    return Column(
      ...
      children: [
        LayerSharePanel(
          ...
          list: LayerItemsList<LayerDataLabel>(
            title: 'Camadas',
