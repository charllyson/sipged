...
class InterativeMap extends StatefulWidget {
  const InterativeMap({super.key});

  @override
  State<InterativeMap> createState() => _InterativeMapState();
}

class _InterativeMapState extends State<InterativeMap> {
  ...
  void _onLineTap(List<TaggedPolyline> tappedLines, TapUpDetails tapDetails) {
    if (tappedLines.isNotEmpty) {
      ...
      setState(() {
        ...
        _tooltipPosition = selected.points.first;
  ...
  Widget _buildInfoPanel() {
    ...
    final entries = _tooltipProperties!.entries.toList();
                  ...
                  rows: [
                    DataRow(
                      cells: [
                        ...
                        _buildTableCell(matchedHighway.extensaoKm.toString() ?? 'Não informado'),
