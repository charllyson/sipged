...
import 'package:siged/_blocs/actives/oacs/active_oacs_state.dart';
import 'package:siged/_blocs/actives/oacs/active_oacs_data.dart';
import 'package:siged/_blocs/actives/oacs/active_oacs_repository.dart';
...
class _OacInspectionsPageState extends State<OacInspectionsPage> {
  ...
  DateTime? _extractNextDateFromNotes(String? notes) {
    // Mantém simples: não tenta parsear. A fonte de verdade é _nextDate.
    return null;
  }
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ActiveOacsCubit, ActiveOacsState>(
      builder: (context, st) {
        ...
        Future<void> onRemoveInspectionAttachment(String inspectionId, Attachment a) async {
          final ok = await _repo.deleteByPath(a.path ?? '');
          if (!ok) return;

          final list = _attachmentsByInspection[inspectionId] ?? <Attachment>[];
          _attachmentsByInspection[inspectionId] = list.where((x) => x.id != a.id).toList();
