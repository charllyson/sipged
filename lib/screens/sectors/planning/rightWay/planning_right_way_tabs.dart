// lib/screens/sectors/planning/rightWay/planning_right_way_tabs.dart
...
// Right Way: form + tabela
import 'package:siged/screens/sectors/planning/rightWay/property/planning_right_way_property_form.dart';
import 'package:siged/screens/sectors/planning/rightWay/property/planning_right_way_property_table.dart';
...
class _TabBarRightWayPageState extends State<TabBarRightWayPage> {
        ...
        body: Stack(
          children: [
            ...
            Padding(
              ...
              child: Column(
                children: [
                  // faixa com o resumo do contrato (opcional)
                  ...
                  // ======= Split: Top (troca com as abas) + Bottom (tabela fixa) =======
                  Expanded(
                    child: LayoutBuilder(
                      builder: (context, constraints) {
                        return Column(
                          children: [
                            // ======= Topo (abas) que encaixa no conteúdo e no máx. 560px =======
                            Flexible(
                              fit: FlexFit.loose,
                              child: Align(
                                alignment: Alignment.topCenter,
                                child: ConstrainedBox(
                                  constraints: const BoxConstraints(maxHeight: 430), // ajuste se quiser
                                  child: TabBarView(
                                    ...
                                    children: [
                                      _wrapWithBlocker(
                                        child: (_propCtrl == null)
                                          ...
                                          child: PlanningRightWayPropertyForm(controller: _propCtrl!),
                                        ),
                                      ),
                                      _wrapWithBlocker(child: Container()),
                                      _wrapWithBlocker(child: Container()),
                                    ],
                                  ),
                                ),
                            ...
                            // ======= Tabela fixa embaixo =======
                            Expanded(
                              child: _wrapWithBlocker(
                                child: (_propCtrl == null)
                                    ? const SizedBox.shrink()
                                    : Padding(
                                  padding: const EdgeInsets.only(bottom: 8.0),
                                  child: PlanningRightWayPropertyTable(
  ...
  /// Bloqueia se não houver `contract.id`
    ...
    if (_contractData?.id == null) {
      return Center(
        child: Padding(
          ...
          child: Text(
            '⚠️ Para acessar esta área, salve primeiro as informações principais do contrato.',
            textAlign: TextAlign.center,
