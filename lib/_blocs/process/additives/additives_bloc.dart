...
class AdditivesBloc extends BlocBase {
  ...
  // -----------------------------
  // Cache de status do DFD por contrato (statusContrato em DFD.identificacao)
  // -----------------------------
  ...
  /// Lê o `statusContrato` diretamente da árvore:
  ...
  Future<List<AdditiveData>> getAdditivesByContractIds(
      ...
      for (final doc in snapshot.docs) {
        ...
        if (data['tipo'] == 'aditivo') {
          ...
          if (originalSnap.exists) {
            final original =
            AdditiveData.fromDocument(snapshot: originalSnap);
            registros.add(
              Registro(
                id: doc.id,
                tipo: 'aditivo',
  ...
  Future<double> getValorPorStatus(
    ...
    // garante cache de statusContrato no DFD
    await _ensureStatusesForContracts(contratos);

    final alvo = statusDesejado.trim().toUpperCase();

    // filtra pelos contratos cujo status (DFD.statusContrato) coincide
  ...
  Future<List<AdditiveData>> getAllAdditivesOfContract({
    ...
    final s = await _db
        ...
        .get();
    return s.docs
        .map((d) => AdditiveData.fromDocument(snapshot: d))
        .toList();
  }
  ...
  Future<void> salvarOuAtualizarAditivo(
      AdditiveData data, String uidContract) async {
  ...
  Future<void> _notificarUsuariosSobreAditivo(
      AdditiveData aditivo, String contractId) async {
  ...
  Future<void> deleteAdditive(
      String uidContract, String uidAditivo) async {
    await _db
        .collection('contracts')
        .doc(uidContract)
        .collection('additives')
