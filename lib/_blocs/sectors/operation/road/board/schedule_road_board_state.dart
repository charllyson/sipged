// lib/_blocs/sectors/operation/road/board/schedule_road_board_state.dart
...
class ScheduleRoadBoardState extends Equatable {
  ...
  // dados
  final List<ScheduleRoadBoardData> services;   // opções de serviço
  final List<ScheduleLaneClass> lanes;          // faixas
  final List<ScheduleRoadBoardData> execucoes;  // células
  ...
  ScheduleRoadBoardState copyWith({
    bool? initialized,
    String? contractId,
    String? summarySubjectContract,
    int? totalEstacas,
    String? currentServiceKey,
    List<ScheduleRoadBoardData>? services,
    List<ScheduleLaneClass>? lanes,
    List<ScheduleRoadBoardData>? execucoes,
  ...
  bool _cellEnabled(ScheduleRoadBoardData e) {
  ...
  ScheduleRoadBoardData get currentServiceMeta {
    if (services.isEmpty) {
  ...
  DateTime? _dateForShade(ScheduleRoadBoardData e) {
  ...
  Color squareColor(ScheduleRoadBoardData e) {
    ...
    if (currentServiceKey == 'geral') {
      if (t == 'concluido' || t == 'em_andamento') {
        ...
        base = (tag.isNotEmpty)
            ? ScheduleRoadBoardStyle.colorForService(tag)
