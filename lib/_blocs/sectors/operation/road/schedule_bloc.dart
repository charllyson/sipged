...
class ScheduleBloc extends Bloc<ScheduleEvent, ScheduleState> {
  ...
  Map<int, Map<int, ScheduleData>> _buildExecIndex(List<ScheduleData> list) {
    final map = <int, Map<int, ScheduleData>>{};
    for (final e in list) {
      (map[e.numero] ??= <int, ScheduleData>{})[e.faixaIndex] = e;
    }
    return map;
  }

  DateTime? _cellDate(ScheduleData e) => e.takenAt ?? e.updatedAt ?? e.createdAt;

  DateTime? _minD(List<ScheduleData> xs) {
    DateTime? d;
    for (final e in xs) {
      final c = _cellDate(e);
      if (c == null) continue;
  ...
  DateTime? _maxD(List<ScheduleData> xs) {
    DateTime? d;
    for (final e in xs) {
      final c = _cellDate(e);
      if (c == null) continue;
  ...
  Future<void> _onApply(
    ...
    try {
      ...
      // Recalcula índice + datas para manter o sombreamento leve
