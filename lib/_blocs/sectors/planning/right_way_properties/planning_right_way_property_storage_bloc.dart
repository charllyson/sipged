...
class PlanningRightWayPropertyStorageBloc extends BlocBase {
  final FirebaseStorage _storage;
  PlanningRightWayPropertyStorageBloc({FirebaseStorage? storage})
      : _storage = storage ?? FirebaseStorage.instance;

  // ====== Paths ==============================================================
  String _sanitize(String s) => s.replaceAll(RegExp(r'[^0-9A-Za-z._-]'), '-');

  /// Base: contracts/{contractId}/planning_highway_domain/properties/{propertyId}/
  ...
  bool _isGeoName(String n) {
    ...
    return s.endsWith('.kml') || s.endsWith('.kmz') || s.endsWith('.geojson');
  }

  /// Lista TODOS os arquivos georreferenciados do contrato,
  /// varrendo cada pasta de propriedade dentro de:
  /// contracts/{contractId}/planning_highway_domain/properties/*/geo/
  Future<List<({String name, String url})>> listarGeoDeTodoContrato({
    required String contractId,
  }) async {
    final base =
    _storage.ref('contracts/$contractId/planning_highway_domain/properties');
    final props = await base.listAll(); // prefixes = pastas de cada propriedade

    final out = <({String name, String url})>[];
    for (final p in props.prefixes) {
      final geoFolder = p.child('geo');
      try {
        final res = await geoFolder.listAll();
        for (final item in res.items) {
          if (_isGeoName(item.name)) {
            try {
  ...
  // ====== Uploads ============================================================
  // RightWayPropertyStorageBloc

  Future<String> uploadGeoWithPicker({
    required String contractId,
    ...
    final result = await FilePicker.platform.pickFiles(
      ...
      allowedExtensions: ['kml', 'kmz', 'geojson'],
      withData: true,
    );
    ...
    final original = result.files.single.name;              // ex.: area.kml
    final safe = _sanitize(original.split('/').last);       // area.kml
    final lower = safe.toLowerCase();

    // pega extensão preservando .kml/.kmz/.geojson
    String ext = '';
    ...
    return _uploadBytesTo(
      ...
      contentType: _guessGeoContentType(safe), // agora funciona
      onProgress: onProgress,
    );
  }


  Future<String> uploadDocWithPicker({
    required String contractId,
    required String propertyId,
    required void Function(double progress) onProgress,
  }) async {
    ...
    return _uploadBytesTo(path,
        bytes: result.files.single.bytes!, contentType: 'application/pdf', onProgress: onProgress);
  }
  ...
  // ====== Listagem/remoção ===================================================
  Future<List<({String name, String url})>> listarGeo({
    required String contractId,
    required String propertyId,
  }) async {
    ...
    for (final item in res.items) {
      try {
        out.add((name: item.name, url: await item.getDownloadURL()));
      } catch (_) {}
    }
    out.sort((a, b) => a.name.compareTo(b.name));
    return out;
  ...
  Future<List<({String name, String url})>> listarDocs({
    required String contractId,
    required String propertyId,
  }) async {
