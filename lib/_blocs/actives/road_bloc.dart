...
import '../../screens/actives/roads/network/styles/stateHighway.dart';

class RoadsBloc extends BlocBase {
  final FirebaseFirestore _db = FirebaseFirestore.instance;
  final Map<String, RoadData> roadDataMap = {};
  String? selectedPolylineId;

  RoadsBloc();

  Future<List<RoadData>> getAllRoads() async {
    final snapshot = await _db.collection('actives_roads').get();
    return snapshot.docs.map((doc) => RoadData.fromDocument(doc)).toList();
  }

  Future<void> saveOrUpdateRoad(RoadData data) async {
  ...
  Future<void> carregarRodoviasDoFirebase() async {
    ...
    for (var doc in snapshot.docs) {
      ...
      final normalizedRoad = RoadData.fromMap(data);
  ...
  List<TappableChangedPolyline> gerarPolylinesEstilizadas({String? selectedId}) {
    ...
    for (final entry in roadDataMap.entries) {
      ...
      final tagId = road.id!;
      final estilo = StateHighway.styleQGISParaStatus(road.stateSurface, 12);
      ...
      final multilinha = estilo.asMap().entries.map((entry) {
        ...
        return TappableChangedPolyline(
          isDotted: false,
          points: StateHighway.deslocarPontos(
            road.points!,
