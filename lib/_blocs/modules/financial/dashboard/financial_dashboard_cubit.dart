...
class FinancialDashboardCubit extends Cubit<FinancialDashboardState> {
  ...
  String? _demandContractIdFromEmpenho(EmpenhoData e) {
    ...
    if (demandContractId != null && demandContractId.trim().isNotEmpty) {
      return demandContractId.trim();
    }

    final contractId = _idToString(e.contractId);
  ...
  String _demandLabelFromEmpenho(EmpenhoData e) {
    final label = e.demandLabel.trim();

    if (label.isNotEmpty) {
      return label;
    }

    final legacy = e.credor.trim();
  ...
  Future<void> loadAll() async {
    emit(
      state.copyWith(
        status: FinancialDashboardStatus.loading,
        clearError: true,
      ),
    );
    ...
    try {
      ...
      emit(
        state.copyWith(
          status: FinancialDashboardStatus.failure,
          error: e.toString(),
        ),
      );
    }
  ...
  Future<
    ...
    final demandContractIds = <String>{};

    for (final e in empenhos) {
      final cid = _demandContractIdFromEmpenho(e);

      if (cid != null && cid.isNotEmpty) {
        demandContractIds.add(cid);
      }
    }

    if (demandContractIds.isEmpty) {
      return (
    ...
    await Future.wait(
      demandContractIds.map((cid) async {
    ...
    for (final ad in allAdditives) {
      ...
      addByContract[cid] =
          (addByContract[cid] ?? 0.0) + (ad.additiveValue ?? 0.0);
    }
    ...
    for (final ap in allApostilles) {
      ...
      apostByContract[cid] =
          (apostByContract[cid] ?? 0.0) + (ap.apostilleValue ?? 0.0);
    }

    final totalByContract = <String, double>{};

    for (final cid in demandContractIds) {
    ...
    for (final e in empenhos) {
      ...
      demandTotals[demand] =
          (demandTotals[demand] ?? 0.0) + (totalByContract[cid] ?? 0.0);
    }
  ...
  void selectEmpenho(String? id) {
    ...
    emit(
      state.copyWith(
        selectedEmpenhoId: v.isEmpty ? null : v,
        clearSelectedEmpenho: v.isEmpty,
      ),
    );
  }

  void setStatusDfdLabel(String v) {
    emit(state.copyWith(statusDfdLabel: v.trim()));
  }
  ...
  void setExtraLabel(String v) {
    emit(state.copyWith(extraLabel: v.trim()));
  }
  ...
  Map<String, Map<String, double>> empenhadoByDemandByFundingSource() {
    ...
    for (final e in state.empenhos) {
      ...
      final source = e.fundingSourceLabel.trim().isNotEmpty
          ? e.fundingSourceLabel.trim()
          : 'Sem fonte';

      out.putIfAbsent(demand, () => <String, double>{});

      out[demand]![source] =
          (out[demand]![source] ?? 0.0) + e.empenhadoTotal;
    }

    final entries = out.entries.toList()
      ..sort((a, b) {
        final at = a.value.values.fold<double>(0.0, (s, v) => s + v);
        final bt = b.value.values.fold<double>(0.0, (s, v) => s + v);
