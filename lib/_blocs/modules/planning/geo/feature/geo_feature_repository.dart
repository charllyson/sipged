...
class GeoFeatureRepository {
  ...
  Future<List<String>> loadFieldNames({
    ...
    for (final doc in snap.docs) {
      final props = GeoFeatureData.resolveProperties(doc.data());
  ...
  (List<GeoFeatureData>, List<ImportColumnMeta>) buildImportedFeatures(
      List<Map<String, dynamic>> rawFeatures,
      ) {
    if (rawFeatures.isEmpty) {
      return (const <GeoFeatureData>[], const <ImportColumnMeta>[]);
    }
    ...
    for (final raw in rawFeatures) {
      final props = GeoFeatureData.resolveProperties(raw);

      keys.addAll(props.keys);

      for (final entry in props.entries) {
        inferredTypes[entry.key] = GeoFeatureData.mergeInferredType(
          inferredTypes[entry.key],
          GeoFeatureData.inferFieldType(entry.value),
        );
      }
    }
    ...
    final features = rawFeatures.map((raw) {
      final feature = GeoFeatureData.fromImportedRawFeature(raw);
  ...
  Future<(List<GeoFeatureData>, List<ImportColumnMeta>)>
  ...
  Future<void> saveFeaturesToCollection({
    ...
    for (int i = 0; i < features.length; i += chunkSize) {
      ...
      for (final feature in chunk) {
        ...
        final geometry =
        GeoFeatureData.encodeGeometryForFirestore(feature.rawGeometry);
