...
class LandPaymentRepository {
  LandPaymentRepository({
  ...
  CollectionReference<Map<String, dynamic>> _collection(String contractId) {
    return _firestore.collection('land').doc(contractId).collection('pagamento');
  }
  ...
  Future<LandPaymentData?> fetchById({
    required String contractId,
    required String propertyId,
  }) async {
    ...
    return LandPaymentData.fromMap(
      map,
      id: doc.id,
      contractId: contractId,
    );
  ...
  Future<LandPaymentData> save(LandPaymentData data) async {
    final propertyId = data.id?.trim();
    if (propertyId == null || propertyId.isEmpty) {
      throw Exception('propertyId é obrigatório para salvar o pagamento.');
    }
