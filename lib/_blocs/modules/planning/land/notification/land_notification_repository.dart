...
class LandNotificationRepository {
  LandNotificationRepository({
  ...
  Future<LandNotificationData?> fetchById({
    required String contractId,
    required String propertyId,
  }) async {
    ...
    return LandNotificationData.fromMap(
      map,
      id: doc.id,
      contractId: contractId,
    );
  ...
  Future<LandNotificationData> save(LandNotificationData data) async {
    final propertyId = data.id?.trim();
    if (propertyId == null || propertyId.isEmpty) {
      throw Exception('propertyId é obrigatório para salvar a notificação.');
    }
