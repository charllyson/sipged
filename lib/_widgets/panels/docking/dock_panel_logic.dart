...
class DockPanelLogic {
  ...
  static bool hasCollapsedRailForArea(
    ...
    return source.any((g) => g.visible && g.collapsed && g.area == area);
  }
  ...
  static List<DockPanelData> projectDocking({
    ...
    final baseGroups = workingGroups.where((g) => g.id != groupId).toList(growable: false);
  ...
  static Offset clampFloatingOffset({
    ...
    final padding = DockPanelConfig.floatingWorkspacePadding;
  ...
  static Size clampFloatingSize({
    ...
    final padding = DockPanelConfig.floatingWorkspacePadding * 2;

    final maxWidth = workspaceSize.width > 0
        ? math.max(
      DockPanelConfig.minFloatingWidth,
      workspaceSize.width - padding,
    )
    ...
    return Size(
      desired.width
          .clamp(
        DockPanelConfig.minFloatingWidth,
        math.min(DockPanelConfig.maxFloatingWidth, maxWidth),
      )
          .toDouble(),
      desired.height
          .clamp(
        DockPanelConfig.minFloatingHeight,
        math.min(DockPanelConfig.maxFloatingHeight, maxHeight),
      )
          .toDouble(),
    );
  }
  ...
  static Rect? resolveDockRectForArea({
    ...
    switch (area) {
      ...
      case DockArea.top:
        final left =
        span == DockCrossSpan.full ? leftRailWidth : leftRailWidth + occupiedLeft;
  ...
  static Rect resolveContentRect({
    ...
    final left =
    (leftInset + contentPadding.left).clamp(0.0, workspaceSize.width).toDouble();

    final top =
    (topInset + contentPadding.top).clamp(0.0, workspaceSize.height).toDouble();
