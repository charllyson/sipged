// lib/_widgets/modals/schedule_modal_controller.dart
...
class ScheduleModalController extends ChangeNotifier {
  ...
  static double _statusToProgress(ScheduleStatus s) {
    switch (s) {
      case ScheduleStatus.aIniciar: return 0;
      case ScheduleStatus.emAndamento: return 50;
      case ScheduleStatus.concluido: return 100;
    }
  }
  ...
  static DateTime? _inferTakenAtFromExistingMetas(Map<String, pm.CarouselMetadata> metas) {
    ...
    for (final m in metas.values) {
      ...
      if (d == null && m.uploadedAtMs != null) {
        try { d = DateTime.fromMillisecondsSinceEpoch(m.uploadedAtMs!); } catch (_) {}
      }
      if (d == null) continue;
  ...
  void _onCommentChanged() {
    if (progress == 0 && commentCtrl.text.trim().isNotEmpty && status == ScheduleStatus.aIniciar) {
  ...
  void save(BuildContext context) {
    ...
    for (final t in targets) {
      final keepUrls = isMulti ? List<String>.from(t.existingUrls) : List<String>.from(_existingUrls);
  ...
  // ——— Feedback do BLoC
  void onBlocStateChanged(BuildContext ctx, {required bool loadingExecucoes, String? error}) {
