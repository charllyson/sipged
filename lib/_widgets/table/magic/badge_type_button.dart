...
class BadgeTypeButton extends StatelessWidget {
  ...
  String _label(bc.ColumnType t) {
    switch (t) {
      case bc.ColumnType.text:
        return 'Texto';
      case bc.ColumnType.number:
        return 'Número';
      case bc.ColumnType.money:
        return r'Monetário (R$)';
      case bc.ColumnType.boolean_:
        return 'Booleano';
      case bc.ColumnType.date:
        return 'Data (DD/MM/YYYY)';
      case bc.ColumnType.auto:
    ...
    return PopupMenuButton<_MenuAction>(
      ...
      onSelected: (a) async {
        ...
        if (a.kind == _MenuKind.remove && onRemove != null) {
          final ok =
          await confirmDialog(context, 'Deseja realmente remover esta coluna?');
          if (ok != true) return;
          onRemove!.call();
        }
      },
      itemBuilder: (ctx) => [
        PopupMenuItem(
          value: _MenuAction.type(bc.ColumnType.text),
          child: Text(_label(bc.ColumnType.text)),
        ),
        PopupMenuItem(
          value: _MenuAction.type(bc.ColumnType.number),
          child: Text(_label(bc.ColumnType.number)),
        ),
        PopupMenuItem(
          value: _MenuAction.type(bc.ColumnType.money),
          child: Text(_label(bc.ColumnType.money)),
        ),
        PopupMenuItem(
          value: _MenuAction.type(bc.ColumnType.boolean_),
          child: Text(_label(bc.ColumnType.boolean_)),
        ),
        PopupMenuItem(
          value: _MenuAction.type(bc.ColumnType.date),
          child: Text(_label(bc.ColumnType.date)),
        ),
        const PopupMenuDivider(),
        PopupMenuItem(
          value: _MenuAction.type(bc.ColumnType.auto),
          child: Text(_label(bc.ColumnType.auto)),
        ),
        if (onRemove != null) ...[
          const PopupMenuDivider(),
          PopupMenuItem(
