...
class PagedTableChanged<T> extends StatefulWidget {
  ...
  // >>> seleção
  final String Function(T item)? getKey;   // ex.: (d) => d.id!
  final String? selectedKey;              // controle externo opcional
  final bool keepSelectionInternally;     // salva seleção dentro do widget
  ...
  /// (columnIndex, ascending, getterDaColuna)
  ...
  // >>> status / empty
  final String? statusLabel; // ex.: "EM ANDAMENTO"
...
class _PagedTableChangedState<T> extends State<PagedTableChanged<T>> {
  ...
  Future<void> _goTo(int page) async {
    ...
    try {
      await widget.onPageChange(page); // pai atualiza currentPage/listData
    } finally {
  ...
  List<_RowChunk<T>> _buildRowChunks(List<T> data) {
    if (widget.groupBy == null) {
      return [ _RowChunk<T>(type: _RowChunkType.normal, items: data) ];
    }
    final map = <String, List<T>>{};
    for (final item in data) {
      final k = widget.groupBy!(item);
      map.putIfAbsent(k, () => []).add(item);
    }
    final chunks = <_RowChunk<T>>[];
    for (final entry in map.entries) {
      chunks.add(_RowChunk<T>(type: _RowChunkType.groupHeader, groupKey: entry.key));
      chunks.add(_RowChunk<T>(type: _RowChunkType.normal, items: entry.value));
    }
    return chunks;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      color: Colors.blueAccent.shade100, // <- fundo do card branco
      elevation: widget.elevation,
      margin: widget.cardMargin,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      child: LayoutBuilder( // <- mede a largura disponível do card
            ...
            children: [
              ...
              if (data.isNotEmpty)
                SingleChildScrollView(
                  ...
                  child: ConstrainedBox( // <- força a tabela a ocupar a largura do card
                      ...
                      child: DataTable(
                        ...
                        rows: chunks.expand((chunk) {
                          if (chunk.type == _RowChunkType.groupHeader) {
                            // Cabeçalho de grupo compatível com/sem leading/actions
                            return [
                              DataRow(
                                color: MaterialStateProperty.all(Colors.grey.shade200),
                                cells: List.generate(totalColumns, (i) {
                                  // primeira coluna: label ou label+key (se não há colunas suficientes)
                                  if (i == 0) {
                                    final label = widget.groupLabel ?? '';
                                    final text = (totalColumns > 1)
                                        ? label
                                        : '$label: ${chunk.groupKey ?? ''}';
                                    return DataCell(Padding(
                                      padding: const EdgeInsets.symmetric(vertical: 6),
                                      child: Align(
                                        alignment: hasLeading ? Alignment.centerRight : Alignment.centerLeft,
                                        child: Text(
                                          text,
                                  ...
                                  // segunda coluna: chave do grupo (se houver)
                                  if (i == 1) {
                                    return DataCell(Padding(
                                      padding: const EdgeInsets.symmetric(vertical: 6),
                                      child: Text(
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      IconButton(
                        tooltip: 'Anterior',
                        onPressed: (!_paging && widget.currentPage > 1) ? () => _goTo(widget.currentPage - 1) : null,
  ...
  List<DataColumn> _buildColumns(bool hasLeading, bool hasActions) {
    ...
    if (hasLeading) {
      cols.add(DataColumn(
        label: Center(child: Text(widget.leadingTitle ?? '')),
      ));
    }
...
/// Suporte interno para "linhas" de grupos e linhas normais
