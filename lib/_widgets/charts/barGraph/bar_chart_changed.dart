...
class _BarChartChangedState extends State<BarChartChanged> {
  ...
  Widget build(BuildContext context) {
    ...
    final totalChartWidth = totalBars * ((widget.widthBar ?? 60) + spacingExtra);
    final chartWidth = totalChartWidth.toDouble(); // canvas = tamanho exato
                  ...
                  BarChartData(
                    ...
                    titlesData: FlTitlesData(
                      bottomTitles: AxisTitles(
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            ...
                            if (idx < 0 || idx >= widget.labels.length) return const SizedBox();
                      ...
                      leftTitles: AxisTitles(
                        sideTitles: SideTitles(
                          showTitles: true,
                          reservedSize: 70, // largura reservada pros números
                          getTitlesWidget: (value, meta) => Padding(
                            padding: const EdgeInsets.only(right: 8), // espaço entre número e barras
                            child: Text(
                    ...
                    barGroups: widget.values.asMap().entries.map((entry) {
                      ...
                      final isHighlighted = widget.highlightedIndexes?.contains(index) ?? false;

                      // se for null, desenha 0, mas com cor "neutra"
                      final double toY = value ?? 0.0;
                      final Color baseColor = widget.barColors?[index] ?? Colors.cyan;
