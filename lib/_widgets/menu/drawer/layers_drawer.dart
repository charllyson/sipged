...
class _LayersDrawerState extends State<LayersDrawer> {
  ...
  bool _existsLayerWithId(List<PlanningLayers> nodes, String id) {
  ...
  bool _areAllChildrenActive(PlanningLayers node) {
    if (!node.isGroup) {
  ...
  bool _hasAnyChildActive(PlanningLayers node) {
    if (!node.isGroup) {
      return widget.activeLayerIds.contains(node.id);
    }
    if (node.children.isEmpty) return false;
  ...
  List<PlanningLayers> _flattenLeaves(PlanningLayers node) {
    if (!node.isGroup) return [node];
    final list = <PlanningLayers>[];
    for (final c in node.children) {
  ...
  Widget _buildGroupRow(
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            // 🔹 Slot vazio para alinhar com a seta/corrente dos itens folha
            ...
            SizedBox(
              ...
              child: Checkbox(
                ...
                tristate: true,
                onChanged: (v) {
                  // Independente se estava em true / false / null,
                  // ao clicar decidimos ligar ou desligar tudo.
                  final checked = v ?? false;
                  final leaves = _flattenLeaves(group);
                  for (final leaf in leaves) {
                    widget.onToggleLayer(leaf.id, checked);
                  }
                },
                activeColor: primaryCheckboxColor, // cor única
            ...
            // 🔹 Setinha de expandir/contrair AGORA NO FIM,
            // alinhada com o ícone de corrente dos layers folha
            IconButton(
              iconSize: 18,
  ...
  Widget _buildLayerRow(
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            // Slot para seta (só grupo usa, aqui é vazio pra alinhar)
            const SizedBox(
              width: 40,
            ),
            ...
            SizedBox(
              ...
              child: Checkbox(
                value: isActive,
                onChanged: (v) =>
                    widget.onToggleLayer(layer.id, v ?? false),
                activeColor: primaryCheckboxColor, // cor única
                visualDensity: VisualDensity.compact,
              ),
            ),
            ...
            IconButton(
              ...
              padding: EdgeInsets.zero,
              visualDensity: VisualDensity.compact,
              onPressed: widget.onConnectLayer == null
                  ? null
                  : () => widget.onConnectLayer!(layer.id),
              icon: Icon(
                Icons.link,
                color: linkIconColor,
              ),
              tooltip: 'Configurar vínculo / serviço',
            ),
          ],
        ),
      ),
