...
class GeoJsonPreviewDialog extends StatefulWidget {
  ...
  final Future<void> Function(
      ...
      Map<String, TypeFieldGeoJson> tipos,
...
class _GeoJsonPreviewDialogState extends State<GeoJsonPreviewDialog> {
  ...
  Map<String, TypeFieldGeoJson> _tipoPorColuna = {};
  ...
  void _processarFeatures() {
    ...
    if (_linhas.isNotEmpty) {
      ...
      for (final col in _colunas) {
        ...
        _tipoPorColuna[col] = TypeFieldGeoJson.string;
      }
  ...
  dynamic _converterValor(dynamic valor, TypeFieldGeoJson tipo) {
    try {
      switch (tipo) {
  ...
  Widget _buildTipoDropdown(String coluna) {
    return DropdownButton<TypeFieldGeoJson>(
      dropdownColor: Colors.white,
      underline: Container(),
      value: _tipoPorColuna[coluna],
      onChanged: (val) => setState(() => _tipoPorColuna[coluna] = val!),
      items: TypeFieldGeoJson.values.map((tipo) => DropdownMenuItem(
        value: tipo,
  ...
  void _renomearColuna(String antigo, String novo) {
    ...
    if (index != -1) {
      setState(() {
        ...
        _tipoPorColuna[novo] = _tipoPorColuna.remove(antigo) ?? TypeFieldGeoJson.string;
      });
    }
  }
